...
class SELayer(nn.Module):
    ...
    def reverse(self, x: List[torch.Tensor]) -> torch.Tensor:
        return x[-1]


class WeightedConcatLayer(nn.Module):
    def __init__(
        self, channel: int, activate: Literal["softmax", "sigmoid"] = "softmax"
    ) -> None:
        ...
        if activate == "softmax":
            ...
            raise ValueError(f"Unsupported activation type: {activate}")
...
class AdaIN(nn.Module):
    ...
    def calc_mean_std(self, feat: torch.Tensor, eps: float = 1e-5) -> tuple:
        size = feat.size()
        assert len(size) == 4
        N, C = size[:2]
    ...
    def forward(
        self, content: torch.Tensor, style_mean: torch.Tensor, style_std: torch.Tensor
    ) -> torch.Tensor:
        assert style_mean is not None and style_std is not None
        ...
        normalized_feat = (content - content_mean.expand(size)) / content_std.expand(
            size
        )
...
class Conv2dBlock(nn.Module):
    def __init__(
        ...
        stride: int,
        padding: int = 0,
        norm: Literal["none", "bn", "in", "ln", "adain", "sn"] = "none",
        activation: str = "relu",
        pad_type: str = "zero",
    ) -> None:
...
class LB(nn.Module):
    def __init__(
        ...
        input_dim: int,
        output_dim: int,
        norm: Literal["none", "bn", "in", "ln", "sn"] = "none",
        activation: Literal["relu", "lrelu", "prelu", "selu", "tanh", "none"] = "relu",
    ) -> None:
...
class MLP(nn.Module):
    def __init__(
        ...
        n_blk: int,
        norm: Literal["none", "bn", "in", "ln", "sn"] = "none",
        activ: Literal["relu", "lrelu", "prelu", "selu", "tanh", "none"] = "relu",
    ) -> None:
        super(MLP, self).__init__()
        self.model: list[nn.Module] = []
        # self.model += [LB(input_dim, dim, norm=norm, activation=activ)]
        self.model.append(LB(input_dim, dim, norm=norm, activation=activ))
        # for i in range(n_blk - 2):
        #     self.model += [LB(dim, dim, norm=norm, activation=activ)]
        for _ in range(n_blk - 2):
            self.model.append(LB(dim, dim, norm=norm, activation=activ))

        self.model.append(
            LB(dim, output_dim, norm="none", activation="none")
...
class StyleEncoder(nn.Module):
    def __init__(
        ...
        dim: int,
        style_dim: int,
        norm: Literal["none", "bn", "in", "ln", "sn"],
        activ: Literal["relu", "lrelu", "prelu", "selu", "tanh", "none"],
        pad_type: Literal["zero", "reflect", "replicate"],
    ) -> None:
...
class HierarchyCoupling(nn.Module):
    def __init__(
        ...
        out_channel: int,
        weight_type: Literal[
            "fixed", "softmax", "sigmoid", "attention", "learned"
        ] = "fixed",
    ) -> None:
        ...
        self.adain = AdaIN()
        self.style_mlp = MLP(
            8, out_channel * 2, out_channel * 3, 3, norm="none", activ="relu"
        )
        ...
        if self.weight_type == "softmax" or self.weight_type == "sigmoid":
            self.weight = WeightedConcatLayer(
                channel=self.splits, activate=self.weight_type
            )
            ...
            raise NotImplementedError(
                "Error: type {} for weight is not implemented.".format(self.weight_type)
            )
    ...
    def reverse(self, input: torch.Tensor, style: torch.Tensor) -> torch.Tensor:
        feature = self.feat
        ...
        for i in range(self.splits - 1, 0, -1):
            tmp_out = self.fixed_weight * (
                tmp_out + input[:, (i - 1) * self.in_channel : i * self.in_channel]
            ) + feature[:, (i - 1) * self.in_channel : i * self.in_channel] * (
                1 - self.fixed_weight
            )
            output_list.append(tmp_out)
...
class HierarchyFlow(nn.Module):
    def __init__(
        ...
        pad_size: int = 10,
        in_channel: int = 3,
        out_channels: List[int] = [30, 120],
        weight_type: Literal["fixed", "softmax", "sigmoid", "attention", "learned"] = "fixed"
    ) -> None:
    ...
    def forward(self, content: torch.Tensor, style: torch.Tensor) -> torch.Tensor:
        ...
        content = content[
            :,
            :,
            self.pad_size : height - self.pad_size,
            self.pad_size : width - self.pad_size,
        ]
        return content
